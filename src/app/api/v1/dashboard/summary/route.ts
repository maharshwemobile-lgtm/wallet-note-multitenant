...
export const GET = withAuth("dashboard.view", async ({ req, user }) => {
  ...
  const [recentThreeD, recentExchanges, pendingSessions, dailyClose] = await Promise.all([
    ...
    prisma.dailyClose.findFirst({
      where: { businessId: user.businessId, date },
    }),
  ]);
  ...
  // POS: today's sales and low-stock alerts
  ...
  return json({
    ...
    dailyCloseStatus: dailyClose?.status ?? "OPEN",
    rates,
    pos: {
